...
import { deleteView, saveProjectsView } from "@/app/(app)/actions/views";
...
export function ProjectsFilters({
  ...
  function applyView(viewId: string) {
    ...
    if (typeof query.health === "string") next.set("health", query.health);
    if (typeof query.ownerId === "string") next.set("ownerId", query.ownerId);
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                ...
                startTransition(async () => {
                  try {
                    await saveProjectsView({ name, query });
            ...
            <Button variant="ghost" className="rounded-2xl" onClick={() => router.push("/projects")}>
              Clear
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="grid gap-2">
            <Label>Health</Label>
            <Select value={current.health} onValueChange={(v) => setParam("health", v)}>
          ...
          <div className="grid gap-2">
            <Label>Owner</Label>
            <Select value={current.ownerId} onValueChange={(v) => setParam("ownerId", v)}>
              <SelectTrigger className="rounded-2xl">
                <SelectValue placeholder="All" />
              </SelectTrigger>
              <SelectContent>
